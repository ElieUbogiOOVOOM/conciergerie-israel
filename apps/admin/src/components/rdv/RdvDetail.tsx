...
/** Fiche détaillée d'un RDV avec actions (statut, attribution, replanification, #36). */
export function RdvDetail({ id }: { id: string }) {
  ...
  return (
    <section aria-label="Fiche rendez-vous" className="flex max-w-5xl flex-col gap-5">
      ...
      {status === "ok" && rdv && (
        <div className="grid grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            <header className="mb-4 flex flex-wrap items-center justify-between gap-3 border-b border-sable pb-4">
              <h1 className="font-title text-h2 text-encre">
                <Link
                  ...
                  className="hover:text-or-profond hover:underline"
                >
          ...
          <RdvActions rdv={rdv} onUpdated={setRdv} />
        </div>
      )}
    </section>
  );
